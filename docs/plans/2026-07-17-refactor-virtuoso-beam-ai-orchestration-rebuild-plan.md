...
- [x] Channel behaviour + web-chat adapter (Phoenix Channels; session identity); ~~FB adapter ported with webhook signature verification~~ + idempotent delivery (at-least-once → exactly-one reply)
      → **Scope change:** FB Messenger dropped — not a relevant 2026 messaging channel (it was only in the plan as the legacy hardwired channel). Built `Virtuoso.Channel` behaviour (`translate_in/1`, `send_out/2`, `verify_webhook/2`) + a Phoenix-free web-chat adapter (session identity → Impression) + a reusable `Virtuoso.Channel.Signature` HMAC-SHA256 verifier (constant-time compare) for any channel that needs webhook auth. Idempotent delivery is already guaranteed by the event log's dedup key. Concrete Phoenix Channels/LiveView *transport* deferred to the optional host app (library core stays Phoenix-free per the Phase 0 library-first decision).
...
#### Phase 2: Ensemble — actor-based parallel consensus (Roemmele concept a) ✅ COMPLETE
...
- [x] Ensemble config surface: per-bot defaults, per-routine overrides (`ensemble: [n: 3, strategy: :majority, models: [...]]`)
      → **Done:** `Virtuoso.Ensemble.Config` (resolve/merge, strategy name→module, per-routine override merged onto per-bot defaults) + `Virtuoso.Bot` (`use Virtuoso.Bot` → `fast/0`, `routines/0`, `ensemble/0` callbacks + generated `routine_registry/0`, `ensemble_for/1`, `responder/1`). A routine is `module` or `{module, ensemble: [...]}`; `Conversation.deliver(imp, responder: MyBot.responder())` runs a message through the bot.
- [x] Dashboard v1: live ensemble runs — votes, dissent, latency, cost per decision
      → **Done:** `examples/dashboard` — a Phoenix LiveView **host app** (library core stays Phoenix-free per the Phase 0 decision), hand-rolled minimal (prebuilt LiveView JS served from deps, no asset pipeline). Fed by new ensemble telemetry: `[:virtuoso, :ensemble, :run, :start|:stop]` with outcome/decision/votes(`count`)/dissent/`usage` (member token usage now aggregated into ensemble meta — cost per decision, previously discarded) + the existing LLM events; a Collector GenServer bridges telemetry → ring buffer → PubSub → LiveView. Offline demo button fires real `Ensemble.run/3` with noisy eval members. Verified end-to-end in a real browser: websocket connected, runs streaming live with votes/dissent (screenshot in PR #71). Auth = Phase 4 (documented dev-only).
...
- [x] libcluster topology (gossip for dev, DNS for Fly.io); **single-node no-op default** so library consumers need zero cluster config
      → **Done:** `Virtuoso.Fabric` — `enabled: false` default resolves the plain `Registry`/`DynamicSupervisor` pair (zero config); enabled resolves the Horde pair (`members: :auto`, `process_redistribution: :active`); `topologies:` config starts libcluster (any strategy — gossip dev, DNSPoll Fly.io), `[]` otherwise. Boot-time config, documented.
- [x] Conversation processes under Horde.Registry + Horde.DynamicSupervisor; handoff = stop → rehydrate from event log on new node (log is source of truth, so split-brain double-send is prevented by dedup on the *outbound* side: send-intent recorded in log before channel send)
      → **Done (mechanism):** `Conversation` addresses via `Fabric.via/lookup/start_child`, so fabric-enabled mode runs conversations under the Horde pair; rehydrate-from-log recovery is mode-independent. `whereis/1` filters locally-dead pids (async registry cleanup) and passes remote pids through to the `:noproc` retry. *Follow-up:* the full kill-node-mid-conversation acceptance drill with real Conversations on peers needs a shared (non-sandbox) DB — tracked for the Phase 3 wrap-up.
- [x] Netsplit conflict resolution: on registry merge, loser process terminates without flushing sends; in-flight LLM tasks abandoned (cost accepted, logged)
      → **Done (with caveat):** `Conversation.start_link` maps `already_started` → `:ignore` (spike finding: the loser's supervisor otherwise crash-loops restarting into the winner's name); bounded backoff in `ensure_started` covers the cleanup window; outbound sends only ever happen through the log's dedup key, so a loser can't double-send by construction. *Caveat:* no dedicated telemetry event yet for abandoned in-flight LLM tasks (cost is accepted but not separately counted) — noted for the dashboard's cost accounting.
- [x] Budget process made fabric-supervised singleton (handoff-capable)
      → **Done:** `Budget.name/0` resolves to the local name (fabric off) or a via tuple through the fabric registry (one budget cluster-wide; API defaults route through it). Fabric mode: every node's boot races a starter task; `start_link` maps `already_started`→`:ignore` so losers drop quietly; failover restarts the singleton on a survivor (counters reset — caps are protective ceilings, not billing records, documented). **Also wired the gate into the pipeline** (closing the enforcement gap): SlowThinking pre-checks the budget per turn (refusal → the defined refusal message, zero member spend) and routes every member call through `with_budget` (records actual usage; mid-turn cap crossings drop members). `record/3` is now a cast (006 partial — halves hot-path round-trips).
- [x] Rolling-deploy policy: versioned `%Impression{}`/state structs + drain-before-upgrade documented
      → **Done:** `docs/operations/rolling-deploys.md` — the three invariants (log-derived state, log-owned exactly-once, versioned envelope), the per-node drain/stop/start procedure incl. the spike's spec-sync caveat and the Budget-singleton note, and cross-release rules for struct/schema/config changes.
