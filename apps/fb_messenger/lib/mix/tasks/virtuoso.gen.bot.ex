defmodule Mix.Tasks.FbMessenger.Gen.Bot do
  @moduledoc """
  Generates executive, routines, and cognition for a Bot.

    mix fb_messenger.gen.bot BotModuleName

  The first argument is the bot's name
  """
  use Mix.Task
  alias Mix.Generator
  alias FbMessenger.Bot
  ...
  def slow_thinking_template(bot_module_name) do
    ...
    defmodule #{bot_module_name}.SlowThinking do
      @moduledoc \"""
      If FastThinking failed to deduce entities and intents then SlowThinking may use a FbMessenger NLP client to determine which routine the bot should execute.
      \"""
